import plusnew, { Props, store, component } from "@plusnew/core/src/index";
...
describe("rendering the elements", () => {
  ...
  it("boolean attribute", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(localState) => <button disabled={localState} />}
      </local.Observer>
    ));
    ...
    expect(container.childNodes.length).toBe(1);
    expect((container.childNodes[0] as HTMLButtonElement).tagName).toBe(
      "BUTTON"
    );
  ...
  it("deleting attribute", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>
        {(localState) => (localState ? <div class="foo" /> : <div />)}
      </local.Observer>
    ));
  ...
  it("move elements", () => {
    ...
    const NestedComponent = component(
      ...
      (Props: Props<{ key: number }>) => (
        <Props>{(props) => <div>{props.key}</div>}</Props>
      )
    );
  ...
  it("move text", () => {
    ...
    const NestedComponent = component(
      "NestedComponent",
      (Props: Props<{ key: number }>) => <Props>{(props) => props.key}</Props>
    );
  ...
  it("update text", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>{(localState) => localState}</local.Observer>
    ));
  ...
  it("removing text", () => {
    ...
    const MainComponent = component("Component", () => (
      <local.Observer>{(localState) => localState && "foo"}</local.Observer>
    ));
